...
**Solution:** Modified `startup.py` to automatically scan all LoRA files in the LoRA directory and add them to the catalog with basic metadata on startup. All new LoRAs are:
- Added to the JSON catalog with minimal metadata
- Marked as `enabled: True` by default
- Marked as `indexed_by_llm: False` (not fully indexed)
- Have `available: True` status

**Files Changed:**
- `startup.py`: Added auto-population logic to scan and add all LoRAs to catalog
...
4. Missing LoRAs are added to the catalog with default values
5. Catalog is saved to `data/lora_index.json`

### Catalog Entry Structure (Non-Indexed)
```json
{
  "file_hash_value": {
